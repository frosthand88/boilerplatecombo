...
import com.boilerplatecombo.service.TimescaleResearcherService;
...
@RestController
@RequestMapping("/timescale/researcher")
public class TimescaleController {

    private TimescaleResearcherService service;

    public TimescaleController(@Lazy TimescaleResearcherService researcherService) {
        this.service = researcherService;
    }

    @GetMapping
    public ResponseEntity<ResearchActivity> getResearchers(
    ...
    @PostMapping
    public ResponseEntity<ResearchActivity> addResearcher(@RequestBody ResearchActivity researcher) {
        ResearchActivity added = service.addResearcher(researcher);
        return ResponseEntity.status(201).body(added);
    }

    @PutMapping("/{id}")
    public ResponseEntity<Void> updateResearcher(@PathVariable Long id, @RequestBody ResearchActivity researcher) {
