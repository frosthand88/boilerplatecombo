...
func NewCockroachService(db *gorm.DB) *CockroachService {
	return &CockroachService{db: db}
}

func (s *CockroachService) GetResearchers(pageNumber, pageSize int, name, sortBy string) ([]models.Researcher2, error) {
	if pageNumber <= 0 {
		pageNumber = 1
	}
	if pageSize <= 0 {
