...
func NewMariaService(db *gorm.DB) *MariaService {
	return &MariaService{db: db}
}

func (s *MariaService) GetResearchers(pageNumber, pageSize int, name, sortBy string) ([]models.Researcher2, error) {
	if pageNumber <= 0 {
		pageNumber = 1
	}
	if pageSize <= 0 {
